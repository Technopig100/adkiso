...
#
# A simple script to run an adkiso image using qemu. The image can be booted
# using BIOS or UEFI.
#
# Requirements:
# - qemu
...
Example:
    ...
    $ run_adkiso -u -i adkiso-2020.05.23-x86_64.iso
EOF
    printf '%s' "${usagetext}"
}
...
display='sdl'
qemu_options=()
working_dir="$(mktemp -dt run_adkiso.XXXXXXXXXX)"
    ...
    while getopts 'abc:dhi:suv' flag; do
        case "$flag" in
            ...
            v)
                ...
                ;;
            *)
                printf '%s\n' "Error: Wrong option. Try 'run_adkiso -h'."
                exit 1
                ;;
        esac
    done
